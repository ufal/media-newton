...
<xsl:stylesheet version='2.0' 
  ...
  <xsl:variable name="cnec-ids" select="//tei:text//*[local-name() = ('name','date','email','num','ref','time','unit')][contains(@ana,'ne:')]/@xml:id" />
  ...
  <xsl:template match="tei:name | tei:date | tei:email | tei:num | tei:ref | tei:time | tei:unit">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <!-- Output CNEC feature (for MISC column) -->
  <!-- tei:name | tei:date | tei:email | tei:num | tei:ref | tei:time | tei:unit -->
  <xsl:template name="CNEC">
    <xsl:variable name="values">
      <xsl:for-each select="ancestor::*[local-name() = ('name','date','email','num','ref','time','unit')]">
        <xsl:variable name="position" select="index-of($cnec-ids, ./@xml:id)"/>
        <xsl:if test="$position">
          <item>
          <xsl:value-of select="replace(@ana,'^.*ne:([^ ]*).*$','$1')"/>
          <xsl:text>_</xsl:text>
  ...
  <!-- Output token id in (for MISC column) -->
  <xsl:template name="ID">
    <xsl:text>|TeiID=</xsl:text>
