...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="handle"></xsl:param>
  <xsl:param name="model-udpipe"><!--TODO--></xsl:param>
  <xsl:param name="model-nametag"><!--TODO--></xsl:param>

  <xsl:output method="xml" indent="yes" encoding="UTF-8" />
  <xsl:preserve-space elements="catDesc seg p"/>

  <xsl:variable name="taxonomies">
    <!--<item>TODO - add type of news taxonomy</item>-->
  ...
  <xsl:variable name="docs">
    <xsl:for-each select="/tei:teiCorpus/xi:include">
      <item>
        ...
        <doc>
          <xsl:apply-templates select="document(concat($inComponentDir, '/', @href))" mode="insertHeader">
            <xsl:with-param name="teiHeader" select="document(concat($inHeaderDir, '/', @href))/tei:TEI/tei:teiHeader"/>
          </xsl:apply-templates>
        </doc>
        <url-new>
  ...
  <xsl:template mode="insertHeader" match="tei:TEI">
    <xsl:param name="teiHeader"/>
    <xsl:copy>
      <xsl:copy-of select="@*"/>
      <xsl:apply-templates select="$teiHeader" mode="insertHeader"/> <!-- předělat na volání templaty a doplnit extent,...-->
      <xsl:copy-of select="./*"/>
    </xsl:copy>
  </xsl:template>
  <xsl:template mode="insertHeader" match="tei:teiHeader">
    <xsl:copy>
      <xsl:apply-templates select="tei:fileDesc" mode="insertHeader"/>
      <encodingDesc>
        <projectDesc>
        </projectDesc>
        <tagsDecl>
  ...
  <xsl:template mode="insertHeader" match="tei:fileDesc">
    <xsl:copy>
